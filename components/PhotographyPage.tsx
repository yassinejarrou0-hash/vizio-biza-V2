...
import type { PhotoContent, Settings, ContactConfig } from "@/lib/content-types";

const CATEGORIES = [
  { label: "Photographie Professionnelle", href: "/services/photo-portrait", icon: "fa-solid fa-user", badge: "Best-seller" },
  { label: "Mariage & Événements", href: "/services/photo-evenements", icon: "fa-solid fa-ring" },
  { label: "Production Vidéo", href: "/services/production-video", icon: "fa-solid fa-video", badge: "Tendance" },
...
const NAV_LINKS = [
  ...
  { label: "Photographie & Vidéo", href: "/photographie", active: true },
...
const DRAWER_LINKS = [
  ...
  { label: "— Photographie Professionnelle", href: "/services/photo-portrait", indent: true },
  { label: "— Mariage & Événements", href: "/services/photo-evenements", indent: true },
  { label: "— Production Vidéo", href: "/services/production-video", indent: true },
  { label: "— Photographie Produit", href: "/services/photo-produit", indent: true },
  { label: "— Photographie Administrative", href: "/services/photo-administrative", indent: true },
  { label: "— Retouche Photo", href: "/services/retouche-photo", indent: true },
  { label: "— Services Studio", href: "/services/services-studio", indent: true },
...
export default function PhotographyPage({ content, settings, contact }: { content: PhotoContent; settings: Settings; contact: ContactConfig }) {
  const PHOTO_HERO_SLIDES = content.heroSlides;
  const PHOTO_SLIDE_COUNT = PHOTO_HERO_SLIDES.length || 1;
  const BEST_HERO_SLIDES = content.bestsellers;
  const BEST_SLIDE_COUNT = BEST_HERO_SLIDES.length || 1;
  const [drawer, setDrawer] = useState(false);
  const [tab, setTab] = useState<"all" | "events" | "video" | "admin">("all");
  ...
  const prevSlide = () => { setActiveSlide(s => (s - 1 + PHOTO_SLIDE_COUNT) % PHOTO_SLIDE_COUNT); setAnimKey(k => k + 1); };
  const nextSlide = () => { setActiveSlide(s => (s + 1) % PHOTO_SLIDE_COUNT); setAnimKey(k => k + 1); };
  const goToSlide = (i: number) => { setActiveSlide(i); setAnimKey(k => k + 1); };
  const slide = PHOTO_HERO_SLIDES[activeSlide] ?? PHOTO_HERO_SLIDES[0];
  ...
  useEffect(() => {
    if (PHOTO_SLIDE_COUNT <= 1) return;
    const id = setInterval(() => {
      setActiveSlide(s => (s + 1) % PHOTO_SLIDE_COUNT);
      setAnimKey(k => k + 1);
    }, 7000);
    return () => clearInterval(id);
  }, [PHOTO_SLIDE_COUNT, activeSlide]);
  useEffect(() => {
  ...
  return (
    <>
      <RevealObserver />
      <Topbar hours="Sur rendez-vous, 7j/7" settings={settings} />
      <Header searchCategories={CATEGORIES} searchPlaceholder="Rechercher une séance…" onMenuToggle={() => setDrawer(true)} settings={settings} />
      <MobileSearch placeholder="Rechercher une séance…" />
      <CategoryNav navLinks={NAV_LINKS} categories={CATEGORIES} settings={settings} />
      <Drawer open={drawer} onClose={() => setDrawer(false)} links={DRAWER_LINKS} ctaLabel="Réserver une séance" settings={settings} />
          ...
          <div className="hero-left">
            <div className="hero-anim" key={animKey}>
              ...
              <div className="hero-cta">
                <a href="#contact" className="btn btn-accent">Réserver une séance <i className="fa-solid fa-arrow-right btn-arrow"></i></a>
              ...
              <div className="hero-bullets">
                ...
                <span className="hero-bullet"><i className="fa-solid fa-palette"></i> Direction artistique</span>
                <span className="hero-bullet"><i className="fa-solid fa-bolt"></i> Livraison rapide</span>
              </div>
            </div>
          </div>
          <div className="hero-right">
            {PHOTO_HERO_SLIDES.map((s, i) => (
              <div key={i} className={`hero-slide${i === activeSlide ? " is-active" : ""}`}>
      ...
      <section className="products" id="products">
        <div className="container">
          <div className="section-head center reveal">
            ...
            <h2>Services en <span>vedette</span></h2>
            <p>Photographie, mariage, vidéo et photo administrative — un studio complet pour tous vos besoins visuels.</p>
          </div>
          <div className="tabs">
            {[
              { id: "all", label: "Best-sellers" },
              { id: "events", label: "Mariage & Événements" },
          ...
          {(["all", "events", "video", "admin"] as const).map((t) => (
            <div key={t} className={`products-grid${tab !== t ? " hidden" : ""}`}>
              {content.products
                .filter((p) => p.tab === t)
                .map((p, i) => (
                  <PCard
          ...
          <div className="hero-left">
            <div className="hero-anim" key={bestAnimKey}>
              ...
              <div className="hero-cta">
                ...
                <a href="#contact" className="btn btn-outline">Réserver une séance</a>
              </div>
            </div>
          </div>
          <div className="hero-right">
          ...
          <div className="about-grid">
            <div className="about-text reveal">
              ...
              <div className="about-features">
                ...
                <div className="about-feat"><i className="fa-solid fa-palette"></i><div><strong>Créativité moderne</strong>Direction artistique</div></div>
      ...
      <ContactSection variant="photography" contact={contact} settings={settings} />
      <Newsletter />
...
function PCard({
  badge, front, back, cat, name, alt, stars = 5,
}: {
  badge?: { kind: "hot" | "new" | "gold" | "pro"; label: string };
  front: string; back: string; cat: string; name: string; alt: string; stars?: number;
}) {
  return (
    <article className="pcard">
      <div className="pcard-media">
        ...
        <div className="pcard-actions">
          ...
          <a href="#contact" className="pcard-action" aria-label="Aperçu"><i className="fa-regular fa-eye"></i></a>
        </div>
        <a href="#contact" className="pcard-quick"><i className="fa-solid fa-clipboard-list"></i> Réserver</a>
      </div>
